...
type analyzerSource struct { receipts map[string]etherscan.RPCReceipt; logs []chainrpc.Log }
func (s *analyzerSource) TransactionReceipt(_ context.Context, hash string) (etherscan.RPCReceipt, error) { value, ok := s.receipts[hash]; if !ok { return etherscan.RPCReceipt{}, chainrpc.ErrPending }; return value, nil }
func (s *analyzerSource) Logs(context.Context, chainrpc.LogFilter) ([]chainrpc.Log, error) { return s.logs, nil }

type analyzerRepository struct { links map[string]store.CrossChainLink }
func (r *analyzerRepository) UpsertCrossChainLink(_ context.Context, link store.CrossChainLink) (store.CrossChainLink, error) { if r.links == nil { r.links = map[string]store.CrossChainLink{} }; r.links[link.IdentityKey] = link; return link, nil }
func (r *analyzerRepository) QueryCrossChainLinks(context.Context, store.BridgeLinkQuery) ([]store.CrossChainLink, error) { result := make([]store.CrossChainLink, 0, len(r.links)); for _, link := range r.links { result = append(result, link) }; return result, nil }

func TestAnalyzerRecognizesAndCompletesETHDeposit(t *testing.T) {
	...
	if err != nil || len(links) != 1 { t.Fatalf("links=%+v err=%v", links, err) }
	link := links[0]
	if link.Status != "completed" || link.Direction != "deposit" || link.SourceAsset != "ETH" || link.SourceAmount != "25" || link.TargetTxHash != "0xtarget" { t.Fatalf("link=%+v", link) }
}
...
func TestAnalyzerRecognizesERC20WithdrawalAndUnknownContract(t *testing.T) {
	...
	if err != nil || len(links) != 1 || links[0].Direction != "withdrawal" || links[0].SourceAsset != "0x000000000000000000000000000000000000000a" || links[0].TargetAsset != "0x000000000000000000000000000000000000000b" { t.Fatalf("links=%+v err=%v", links, err) }
	links, err = analyzer.Analyze(context.Background(), "base", "0xunknown")
	if err != nil || len(links) != 0 { t.Fatalf("unknown links=%+v err=%v", links, err) }
}
