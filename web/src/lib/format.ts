...
export function formatChainAmount(raw: string | undefined, decimals?: number, maxFractionDigits = DETAIL_AMOUNT_FRACTION_DIGITS): string {
  if (!raw) return "0";
  const negative = raw.startsWith("-");
  const digits = negative ? raw.slice(1) : raw;
  if (!/^\d+$/.test(digits)) return raw;
  if (decimals === undefined || decimals < 0) return digits.length > 12 ? scientificInteger(digits, negative, 0, maxFractionDigits) : raw;
  if (decimals === 0) return raw;
  const visibleDecimals = Math.min(decimals, Math.max(0, Math.trunc(maxFractionDigits)));
  ...
  if (rounded === 0n && BigInt(digits) !== 0n) return scientificInteger(digits, negative, -decimals, maxFractionDigits);
  return `${negative && rounded !== 0n ? "-" : ""}${groupedWhole}${roundedFraction ? `.${roundedFraction}` : ""}`;
}

export function formatAssetAmount(raw: string | undefined, decimals: number | undefined, symbol: string | undefined, maxFractionDigits = DETAIL_AMOUNT_FRACTION_DIGITS): string {
  const amount = formatChainAmount(raw, decimals, maxFractionDigits);
  return symbol ? `${amount} ${symbol}` : amount;
}

function scientificInteger(digits: string, negative: boolean, exponentOffset: number, maxFractionDigits: number): string {
  const significant = digits.replace(/^0+/, "");
  if (!significant) return "0";
  const mantissaDigits = significant.slice(0, Math.max(1, Math.trunc(maxFractionDigits) + 1));
