...
describe("API authentication", () => {
  ...
  it("encodes the transaction hash route", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      "/api/v1/transactions/0xabc?chain=ethereum",
    );
  });
  ...
  it("looks up the latest sync job by chain and address", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await api.latestSyncJob(
      "ethereum",
      "0x0000000000000000000000000000000000000001",
    );
    expect(fetchMock.mock.calls[0][0]).toBe(
      "/api/v1/sync-jobs/latest?chain=ethereum&address=0x0000000000000000000000000000000000000001",
    );
  });
  ...
  it("looks up the latest trace job by the complete trace query", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ id: "trace-1" }), { status: 200 }),
      );
    vi.stubGlobal("fetch", fetchMock);
    await api.latestTraceJob({
      chain: "ethereum",
      ...
      Promise.resolve(
        new Response(
          path.endsWith("job-3")
            ...
            : JSON.stringify({ jobId: path.split("/").pop() }),
          { status: path.endsWith("job-3") ? 429 : 200 },
        ),
      ),
    );
    vi.stubGlobal("fetch", fetchMock);
    const promise = api.syncJobs(
      Array.from({ length: 7 }, (_, index) => `job-${index + 1}`),
    );
