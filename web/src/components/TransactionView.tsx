    ...
    <div className="analysis-grid">
      <section><h3><GitBranch size={15}/> Swap 路径</h3>
        {analysis.swaps.length === 0 ? <p className="empty-copy">未发现经过官方 Factory 验证的 Uniswap V3 Swap。</p> : analysis.swaps.map((swap, index) =>
          <article className={`swap-row ${swap.verified ? "verified" : "unknown"}`} key={`${swap.pool}-${swap.logIndex}`}>
            <div className="swap-index">{index + 1}</div><div className="swap-body">
              ...
              <div className="swap-flow"><code title={swap.amountIn}>{swap.amountIn ? formatChainAmount(swap.amountIn) : "?"}</code><small>{swap.tokenIn ? short(swap.tokenIn) : "未知资产"}</small><ArrowRight size={16}/><code title={swap.amountOut}>{swap.amountOut ? formatChainAmount(swap.amountOut) : "?"}</code><small>{swap.tokenOut ? short(swap.tokenOut) : "未知资产"}</small></div>
        ...
        {analysis.transfers.length === 0 ? <p className="empty-copy">Receipt 中没有 ERC-20 Transfer 日志。</p> : analysis.transfers.map(transfer => <div className="transfer-fact" key={`${transfer.token}-${transfer.logIndex}`}>
          <a href={explorer("address", transfer.token)} target="_blank" rel="noreferrer">{short(transfer.token)}</a><code title={transfer.amount}>{formatChainAmount(transfer.amount)}</code><span>{short(transfer.from)} <ArrowRight size={11}/> {short(transfer.to)} · log #{transfer.logIndex}</span>
        </div>)}
      </section>
