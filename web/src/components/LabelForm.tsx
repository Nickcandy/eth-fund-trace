...
interface Props {
  chain: Chain;
  address: string;
  onSubmit: (input: LabelInput) => Promise<boolean | void>;
}
...
export function LabelForm({ chain, address, onSubmit }: Props) {
  ...
  return (
    <form className="label-form" onSubmit={submit}>
      <label>
        <span>标签类型</span>
        <input name="type" required placeholder="标签类型，如 exchange" />
      </label>
      <label>
        <span>来源</span>
        <select name="source" aria-label="来源">
          <option value="manual">人工</option>
          <option value="public-list">公开名单</option>
        </select>
      </label>
      <label>
        <span>风险等级</span>
        <select name="risk" aria-label="风险等级">
          <option value="">无风险级别</option>
      ...
      <label>
        <span>证据</span>
        <textarea name="evidence" placeholder="证据，每行一条" />
      </label>
      <label>
        <span>备注</span>
      ...
      <button type="submit" disabled={state === "saving"}>
        {state === "saving" ? "保存中" : "保存标签"}
      </button>
      {message && (
        <p
          className={`label-form-message ${state}`}
