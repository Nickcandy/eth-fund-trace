...
export function QueryBar({ value, onChange, onSubmit, busy, error }: Props) {
  const submit = (event: FormEvent) => { event.preventDefault(); onSubmit(); };
  return (
    <form className="query-bar" onSubmit={submit} aria-label="资金链路查询">
      <label className="address-field">
        <span>分析地址</span>
        <input aria-label="分析地址" value={value.address} onChange={(event) => onChange({ ...value, address: event.target.value.trim() })} placeholder="0x..." spellCheck={false} />
      </label>
      <label><span>网络</span><select aria-label="网络" value={value.chain} onChange={(event) => onChange({ ...value, chain: event.target.value as TraceQuery["chain"] })}><option value="ethereum">Ethereum</option><option value="base">Base</option></select></label>
