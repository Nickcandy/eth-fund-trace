...
- 地址、链、方向、深度和 Top-N 参数校验，根资产为 ETH 与链级官方 ERC-20 白名单；
- `202` 异步任务协议；
...
- 根节点查询原生 ETH 与链级官方 ERC-20 白名单；Mongo 分别为每个地址、方向和资产按累计整数金额选择 Top-N 对手方。

注意：Top-N 只在同一资产内按金额排名，不跨 ETH 与不同 Token 比较。只有经过验证的合约兑换才会切换分支资产。
